...
class GS1DigitalLinkToolkit {
  ...
  analyseURI(gs1DigitalLinkURI, extended) {
    ...
    const pcl = pathComponents.length;
    let pci = pcl;
    let searching = true;
    ...
    const pathElements = relevantPathComponents;
  ...
  extractFromGS1digitalLink(gs1DigitalLinkURI) {
    ...
    const splitPath = uriPathInfo.split('/').slice(1);
    ...
    if (this.pathSequenceConstraints.hasOwnProperty(aiSeq[0])) {
      ...
      for (let j = 1; j < aiSeq.length; j++) {
        const i = this.pathSequenceConstraints[aiSeq[0]].indexOf(aiSeq[j]);
        if (i <= lastIndex) {
  ...
  extractFromCompressedGS1digitalLink(gs1DigitalLinkURI) {
    ...
    let { queryString } = s;
    let { uriPathInfo } = s;
    const candidates = s.pathCandidates;
    const { queryStringCandidates } = s;
    const nonGS1queryStringCandidates = {};
  ...
  buildGS1elementStrings(gs1AIarray, brackets) {
    ...
    if (brackets == true) {
      if (identifiers.length !== 1) {
        ...
        for (const a in sortedAttributes) {
  ...
  buildBinaryValue(charstr, n, alphabet) {
    ...
    let binValue = '';
    const binLength = n * charstr.length;
  ...
  handleDecodings(enc, binstr, cursor, gs1AIarray, key, numChars) {
    ...
    switch (enc) {
      case 0:
        ...
        const s = BigInt(`0b${rbv}`).toString();
        gs1AIarray[key] += s;
        break;
    ...
    function buildString(numChars, alphabet, cursor, multiplier, binstr) {
      ...
      const rv = {};
      let s = '';
      const numBitsForValue = multiplier * numChars;
      const rbv = binstr.substr(cursor, numBitsForValue);
      cursor += numBitsForValue;
    ...
    while (totallength - cursor > 8) {
      ...
      if (d1 >= 0 && d2 >= 0 && d1 <= 9 && d2 <= 9) {
        ...
        if (this.tableOpt.hasOwnProperty(h1h2)) {
          ...
          const bitsIncludingF = binstr.substr(cursor - 8, 11);
  ...
  compressGS1AIarrayToBinary(gs1AIarray, useOptimisations, nonGS1keyvaluePairs) {
    ...
    if (useOptimisations) {
      const akeys = JSON.stringify(akeysa);
      ...
      do {
        ...
        const v = this.tableOpt[bestCandidate];
    ...
    if (this.tableF.hasOwnProperty(key)) {
      ...
      for (const j in this.tableF[key]) {
        ...
        if (tx.hasOwnProperty('L') && tx.E == 'N') {
          ...
          const binValue = this.padToLength(BigInt(charstr).toString(2), this.numberOfValueBits(tx.L));
          binstr += binValue;
        }
        ...
        if (tx.hasOwnProperty('M') && tx.E == 'N') {
          ...
          const binValue = this.padToLength(BigInt(charstr).toString(2), this.numberOfValueBits(charstr.length));
