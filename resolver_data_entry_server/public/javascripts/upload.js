...
const convertValidationCodeToText = (validationCode) =>
    Object.keys(global_entryResponseStatusCodes)
        .find((key) => global_entryResponseStatusCodes[key] === parseInt(validationCode));
...
const selectExcelSpreadSheet = () =>
{
    ...
    for (const sheetName of global_data_ExcelWorkBook.SheetNames)
    {
        ...
        if (!global_officialGS1Template || sheetName.trim()
            .toUpperCase() === sheetName.trim())
        {
...
/**
 * This function detects JavaScript code (such as alert() or console commands) in an incoming string, return true if
 * JavaScript id found, else false. WARNING: It is not possible to detect all JavaScript code, so this function is not
 * 100% reliable.
 */
const detectJavaScriptCode = (dataLine) =>
{
    ...
    for (const jsCodeItem of jsCode)
    {
        if (dataLine.trim()
            ...
            .includes(jsCodeItem))
        {
            console.log('javascript code detected: "' + jsCodeItem + '" found in data line: ' + dataLine);
            return true;
        }
    }
    return false;
...
const showDataFromCSVFile = (officialExcelSpreadsheetWasSourceFlag) =>
{
    ...
    for (const rawDataLine of global_dataLines)
    {
        ...
        // This section only runs on the first column, which is  usually a header column.
        if (firstColumnFlag)
        {
            htmlTable += '<tr>';
...
const performResolverEntryCheck = async (resolverEntry, divCheckResults, counter, dataLine, dataOKFlag, resolverResponse) =>
{
    ...
    if (resolverEntry.identificationKeyType === '')
    {
        ...
        {
            resolverEntry.identificationKeyType = document.getElementById('selectIdentificationKeyType')
                .value
                .toLowerCase();
            if (resolverEntry.identificationKeyType === '')
            {
    ...
    else
    {
        ...
        // We check if it is a URI or CURIE. If so, there is nothing more we need to do as resolverResponse.linkType is
        // 'API ready'. If isTitle instead (which we test lowercase/trimmed versions for max compatibility, we quietly
        // convert the linkType to its CURIE value. If not any of the three, it's an erro
        const isCURIE = global_linkTypes.find((linkType) => linkType.curie.toLowerCase()
            .trim() === resolverResponse.linkType.toLowerCase()
            .trim());
        const isURI = global_linkTypes.find((linkType) => linkType.url.toLowerCase()
            .trim() === resolverResponse.linkType.toLowerCase()
            .trim());
        const isTitle = global_linkTypes.find((linkType) => linkType.title.toLowerCase()
            .trim() === resolverResponse.linkType.toLowerCase()
            .trim());
        ...
        {
            // It's unlikely we would end up here but we have code to deal with this as we are definitely in a data
            // error.
